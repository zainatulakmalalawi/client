...
public class RawUdpPacketSender {
	...
	public void sendPacket (URI destination, byte[] packet) throws IOException {
	...
	private PcapIf getPcapDevice() {
		...
		if (r == Pcap.NOT_OK || allDevs.isEmpty()) {
			logger.log(Level.SEVERE, String.format ("Can't read list of devices, error is %s", errorBuffer.toString()));
			return null;
		}
	...
	private void sendPacket(byte[] destinationAddress, int port, byte[] data) throws IOException {
		...
		if (pcap.sendPacket(packet) != Pcap.OK) {
			throw new IOException(String.format("Failed to send UDP packet with error: %s", pcap.getErr()));
		}
	}

	priate byte[] hexStringToByteArray(String s) {
		intlen = s.length();
		byte[] data = ne byte[len / 2];
